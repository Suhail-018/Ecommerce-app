...
const Navbar = () => {
  ...
  return (
    <div className='flex items-center justify-between py-5 font-medium'>
        <Link to='/'> <img  src={assets.logo2} className='w-36' alt="logo" /> </Link>
        ...
        {/* space between 3 logo main menu div and last search and other icons */}
        <div className='flex items-center gap-6'>
                <img onClick={ ()=> setShowSearch(true)}  src={assets.search_icon} className='w-5 cursor-pointer'alt='search'/> 
                <div className='group relative'>
                    <img onClick={()=> token ? null : navigate('/login')} src={assets.profile} className='w-5 cursor-pointer'alt='contact'/>
                    {/* dropdown menu */}
                    {token &&        
